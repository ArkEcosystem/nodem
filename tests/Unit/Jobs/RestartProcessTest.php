...
it('should send a request to restart the process', function (): void {
    ...
    (new RestartProcess($user, $process))->handle();
...
it('logs the process', function (): void {
    ...
    expect($lastLoggedActivity->description)->toEqual('Restarted Relay process');
...
it('sets a pending status when job is created', function (): void {
    ...
    new RestartProcess($user, $process);

    expect($process->fresh()->status)->toEqual(ProcessStatusEnum::WAITING_RESTART);
});
...
it('gets the status from the server if request fails', function (): void {
    ...
    $job = new RestartProcess($user, $process);
...
it('stores the error details on the failed task store', function (): void {
    ...
    Cache::shouldReceive('get')
        ...
        ->with(
            'alerts-'.$user->id,
            [
                new Alert(AlertType::RESTART_SERVER, 'warning', $server->name),
            ],
            Carbon::class,
        );
